...
export default function DrawCanvas({ ...props }: Props) {
  ...
  useEffect(() => {
    ...
    initSocket().catch((err) => console.log(err));
  ...
  async function initSocket() {
    ...
    socket.on("newDrawCollab", (data) => {
      ...
      if (lastLineCollabed && lineCollabUser.length > 0) {
    ...
    <div className="relative h-full w-full">
      ...
      >
        <Layer>
          ...
          {collabLines.current.map((line: CollabLineDraw, i: number) =>
...
function Cursor({ ...props }: mouseCollabUser) {
  return (
    <div
      key={props.id}
      className="absolute z-30 flex h-5 w-5 transition-all"
      style={{
        top: `${props.y}px`,
        left: `${props.x}px`,
      }}
    >
      <svg
        className="h-full w-full fill-blue-700 text-blue-700 "
        viewBox="0 0 21 22"
        fill="none"
